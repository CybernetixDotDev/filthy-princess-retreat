...
function harness() {
  ...
  const returnExports = {} as { authReturnPath: (next?: string | null, returnTo?: string | null) => string };
  runInNewContext(ts.transpileModule(readFileSync("lib/auth-return.ts", "utf8"), {compilerOptions:{module:ts.ModuleKind.CommonJS}}).outputText, {exports:returnExports,require:()=>({safeNextPath})});
...
test("Google uses Supabase OAuth and preserves existing destinations without referral metadata",async()=>{
  ...
  for(const next of paths){const form=new FormData();form.set("returnTo",next);await assert.rejects(actions.signInWithGoogle({},form),/REDIRECT:https:\/\/provider.example/);const call=h.calls.at(-1)!;assert.equal(call.provider,"google");const options=call.options as {redirectTo:string;skipBrowserRedirect:boolean};const url=new URL(options.redirectTo);assert.equal(url.origin,"http://localhost:3000");assert.equal(url.pathname,"/auth/callback");assert.equal(url.searchParams.get("returnTo"),next);assert.equal(options.skipBrowserRedirect,true);assert.deepEqual(Object.keys(call).sort(),["options","provider"]);}
  const bad=new FormData();bad.set("next","https://evil.example");await assert.rejects(actions.signInWithGoogle({},bad),/REDIRECT/);assert.equal(new URL((h.calls.at(-1)!.options as {redirectTo:string}).redirectTo).searchParams.get("returnTo"),"");
});
