...
function harness() {
  const h={calls:[] as Record<string,unknown>[],passwordCalls:0,exchanges:[] as string[],fail:false,throws:false};
  const client={auth:{
  ...
  const mocks:Record<string,unknown>={"@/lib/supabase/server":{createClient:async()=>client},"@/lib/domain":{safeNextPath},"next/navigation":{redirect:(url:string)=>{throw Error(`REDIRECT:${url}`);}}};
...
test("Google uses Supabase OAuth and preserves existing destinations without referral metadata",async()=>{
  ...
  const bad=new FormData();bad.set("next","https://evil.example");await assert.rejects(actions.signInWithGoogle({},bad),/REDIRECT/);assert.equal(new URL((h.calls.at(-1)!.options as {redirectTo:string}).redirectTo).searchParams.get("next"),"/inner-sanctum");
});
