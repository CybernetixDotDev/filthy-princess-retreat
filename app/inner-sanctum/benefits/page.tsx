...
function Benefit({ benefit, past = false }: { benefit: SignedBenefit; past?: boolean }) {
  const canRespond = !past && !benefit.response && RESPONDABLE_BENEFIT_TYPES.includes(benefit.type as (typeof RESPONDABLE_BENEFIT_TYPES)[number]);
  return <article className="sanctum-benefit-item">
    ...
    <div>{benefit.eyebrow ? <p className="sanctum-eyebrow">{benefit.eyebrow}</p> : <p className="sanctum-eyebrow">{benefitLabel(benefit.type)}</p>}<h2>{benefit.title}</h2><div className="sanctum-prose">{benefit.body.split("\n\n").map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
      {benefit.response ? <p className="sanctum-response">You {benefit.response} this.</p> : null}
      {benefit.cta_href && benefit.cta_label ? <Link className="sanctum-link" href={benefit.cta_href}>{benefit.cta_label}</Link> : null}
