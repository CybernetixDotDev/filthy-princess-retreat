...
export default async function EditCollectiblePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><div className="admin-title"><div><p className="eyebrow">Collectible</p><h1>{collectible.title}</h1></div></div>
    ...
    <section className="admin-panel"><h2>Grant to a member</h2>{collectible.status === "active" ? <form action={grantCollectible.bind(null, collectible.id)} className="form-grid"><label className="full-span">Member<select name="user_id" required defaultValue=""><option value="" disabled>Select a member</option>{members?.map((member) => <option key={member.user_id} value={member.user_id}>{member.email ?? member.user_id}{owned.has(member.user_id) ? " — already owned" : ""}</option>)}</select></label><div className="full-span"><SubmitButton>Grant collectible</SubmitButton></div></form> : <p className="admin-caution">This collectible is {collectible.status}. Set it to Active before granting it to a member.</p>}</section>
  </>;
}
