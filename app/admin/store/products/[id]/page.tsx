...
import { storeProductImageUrl } from "@/lib/store";
...
export default async function EditStoreProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><div className="admin-title"><div><p className="eyebrow">Store product</p><h1>{product.name}</h1></div></div><section className="admin-panel"><AdminStoreProductForm action={action} product={product} imageUrl={storeProductImageUrl(state.supabase, product.image_path)} /></section></>;
}
