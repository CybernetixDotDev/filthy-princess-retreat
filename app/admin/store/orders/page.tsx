...
export default async function AdminStoreOrdersPage() {
  ...
  return <><div className="admin-title"><div><p className="eyebrow">Store</p><h1>Orders</h1></div></div><section className="admin-panel"><div className="table-wrap"><table><thead><tr><th>Order</th><th>Created</th><th>Buyer</th><th>Product</th><th>Total</th><th>Status</th></tr></thead><tbody>{orders?.map((order) => <tr key={order.id}><td><Link className="text-link" href={`/admin/store/orders/${order.id}`}>{order.order_reference}</Link></td><td>{new Date(order.created_at).toLocaleString("en-ZA")}</td><td>{order.buyer_email}</td><td>{productNames.get(order.id) ?? "—"}</td><td>{formatStoreMoney(Number(order.total_amount), order.currency)}</td><td>{commercialStoreOrderLabel(order.status)}</td></tr>)}{!orders?.length ? <tr><td colSpan={6}>No Store orders yet.</td></tr> : null}</tbody></table></div></section></>;
}
