...
export default async function SignInPage({ searchParams }: { searchParams: Promise<{ mode?: string; next?: string; authError?: string }> }) {
  const { user, isAdmin } = await getAuthState();
  const query = await searchParams;
  const next = safeNextPath(query.next, "/inner-sanctum");
  if (user) redirect(query.next ? next : isAdmin ? "/admin" : next);
